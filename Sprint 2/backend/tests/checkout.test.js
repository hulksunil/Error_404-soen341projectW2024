...
test('testing createCheckout', async () => {
  const checkout = await CheckoutDB.createCheckout(
    'reservationId',
    'trait',
    'action'
  )
...
test('testing findAllCheckouts', async () => {
  const createdCheckout1 = await CheckoutDB.createCheckout(
    'reservationId1',
    'trait1',
    'action1'
  )

  const createdCheckout2 = await CheckoutDB.createCheckout(
    'reservationId2',
    'trait2',
    'action2'
  )
...
test('testing findCheckoutById', async () => {
  const createdCheckout = await CheckoutDB.createCheckout(
    'reservationId',
    'trait',
    'action'
  )
...
test('testing deleteAllCheckout', async () => {
  await CheckoutDB.createCheckout('reservationId1', 'trait', 'action')
  await CheckoutDB.createCheckout('reservationId2', 'trait', 'action')
